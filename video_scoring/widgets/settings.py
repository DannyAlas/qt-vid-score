...
from video_scoring.settings import Scoring, Playback, KeyBindings, ProjectSettings, AbstSettings
if TYPE_CHECKING:
...
class SettingsDockWidget(QtWidgets.QDockWidget):
    ...
    def populate_key_bindings(self):
        ...
        for action_name, key_sequence in self.main_win.project_settings.key_bindings.__dict__.items():
            ...
            widget.setToolTip(self.main_win.project_settings.key_bindings.help_text()[action_name])
            widget.keySequenceChanged.connect(self.key_sequence_changed)

    def populate_tab(self, tab_widget: QtWidgets.QTabWidget, pyd_model: Union[Scoring, Playback, ProjectSettings]):
        # create a widget for the tab
        tab = QtWidgets.QWidget()
        tab_layout = QtWidgets.QVBoxLayout()
        tab.setLayout(tab_layout)
        ...
        for field_name in pyd_model.model_fields:
            if field_name in ['scoring', 'playback', 'key_bindings', 'projectSettings', 'timestamps']:
                continue
            # create a label for the field
            ...
            help_text = [f"<b>{i.replace('_', ' ').title()}<b>" if i.__contains__('_') else i for i in help_text]
            # fix the above line 
            ...
            if pyd_model.__class__.__name__ == 'ProjectSettings':
                ...
                raise ValueError(f"pyd_model must be one of ProjectSettings, Playback, or Scoring, not {pyd_model.__class__.__name__}")
            ...
            if widget is not None:
                ...
                try:
                    widget.valueChanged.connect(lambda value: self.settings_changed(value, pyd_model))
                except AttributeError:
                    pass
                try:
                    widget.currentTextChanged.connect(lambda value: self.settings_changed(value, pyd_model))
                except AttributeError:
                    pass
                try:
                    widget.textChanged.connect(lambda value: self.settings_changed(value, pyd_model))
                except AttributeError:
                    pass                

    def settings_changed(self, value, pyd_model: Union[Scoring, Playback, ProjectSettings]):
        widget = self.sender()
        if not widget:
            return
        if pyd_model.__class__.__name__ == 'ProjectSettings':
            ...
            self.main_win.project_settings.playback.__setattr__(widget.objectName(), value)
            print(self.main_win.project_settings.playback.__dict__)
        elif pyd_model.__class__.__name__ == 'Scoring':
            self.main_win.project_settings.scoring.__setattr__(widget.objectName(), value)
    ...
    def key_sequence_changed(self, value: QtGui.QKeySequence):
        ...
        if not widget:
            return
        self.main_win.project_settings.key_bindings.__setattr__(widget.objectName(), value.toString())
