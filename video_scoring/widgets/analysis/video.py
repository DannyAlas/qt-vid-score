...
class Video:
    ...
    def downsample_frame(self, frame, scale_factor, interpolation=cv2.INTER_AREA):
        return cv2.resize(
            frame, (0, 0), fx=scale_factor, fy=scale_factor, interpolation=interpolation
        )
...
class VideoAnalysisDock(QtWidgets.QDockWidget):
    ...
    def load_video(self):
        path = QtWidgets.QFileDialog.getOpenFileName(
            self,
            "Select Video",
            filter="Video Files (*.mp4 *.avi *.mov *.mkv *.wmv *.flv *.webm *.m4v *.mpg *.mpeg *.m2v *.3gp *.3g2 *.mxf *.roq *.ts *.mts *.m2ts *.vob *.gif *.gifv *.mng *.avi *.mov *.qt *.wmv *.yuv *.rm *.rmvb *.asf *.amv *.mp4 *.m4p *.m4v *.mpg *.mp2 *.mpeg *.mpe *.mpv *.mpg *.mpeg *.m2v *.m4v *.svi *.3gp *.3g2 *.mxf *.roq *.nsv *.flv *.f4v *.f4p *.f4a *.f4b)",
        )
        if path:
